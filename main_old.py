...
def disable_view_items(view: Optional[discord.ui.View]) -> None:
    """Disable every component without depending on a custom view method."""
    if view is None:
        return
...
class LiveMonitor:
    ...
    async def run(self) -> None:
        ...
        try:
            while not self.stopped and self.message_available and time.monotonic() < deadline:
                await self.refresh_and_edit()
                if self.stopped or not self.message_available:
                    break
            ...
            if self.message_available and not self.finalized:
                try:
                    await self.edit_message(final=True)
                except discord.NotFound:
                    # The user or a moderator deleted the dashboard message.
                    self.message_available = False
                    log.info("Live monitor message %s was deleted before finalization", self.message.id)
                except Exception:
    ...
    async def refresh_and_edit(self) -> None:
        ...
        try:
            ...
            # A deleted message should stop the background loop cleanly instead of
            # producing an exception every ten seconds.
            self.message_available = False
            self.stopped = True
            active_monitors.pop(self.message.id, None)
            log.info("Live monitor message %s no longer exists; stopping monitor", self.message.id)
...
class StopButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        self.monitor.stop()
        disable_view_items(self.monitor.view)
        await interaction.response.defer(thinking=False)
        try:
            await self.monitor.edit_message(final=True)
        except discord.NotFound:
            self.monitor.message_available = False
...
async def start_live_monitor(
    ...
    # Use a normal channel message, not an interaction-webhook message. Discord
    # interaction tokens are temporary, while a normal bot message can be edited
    # for the full 27-minute monitor and beyond.
    channel = interaction.channel
    if channel is None or not hasattr(channel, "send"):
        webhook_message = await interaction.followup.send(embed=starting_embed, wait=True)
        try:
        ...
        await interaction.followup.send(
            f"Live lobby monitor started: {message.jump_url}",
            ephemeral=True,
        )
    monitor = LiveMonitor(
        message=message,
